...
ArangoDB = (function() {
  ...
  ArangoDB.prototype.destroyAll = function(model, callback) {
    var query, resultVariable;
    debug("------------------------------");
    debug("" + model + ": DELETE all instance");
    query = this.db.query["new"]();
    resultVariable = model.toLowerCase();
    query = query["for"](resultVariable)["in"](model).remove(
      resultVariable)["in"](model);
    return this.executeQuery(query.toString(), {}, function(err) {
      if (err) {
        return callback(err);
      }
      return callback(null);
